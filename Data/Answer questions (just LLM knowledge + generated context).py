...
for model_name, model_data in models.items():
    ...
    for index, row in df_quiz.iterrows():
        ...
        input_text = f"Generate a context based on this law field: {ref.iloc[0]['article_text']}"
        context = nlp(input_text, max_new_tokens=context_window)[0]['generated_text']
        context = extract_reply(context)

        # Generate the system prompt               
        input_text = prompt_function(
            f"You are an expert in the field of law. Based on this context: {context}\nChoose the correct answer to the question below:",
